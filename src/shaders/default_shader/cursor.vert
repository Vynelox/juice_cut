 ...
 *
 *   Fill mode    (u_rainbow < 0.5) : 3 vertices = 1 triangle
 *   Outline mode (u_rainbow >= 0.5): 3 vertices = LINE_LOOP
 *
 * Uniforms:
 ...
 *   u_rainbow     — 0.0 = fill, 1.0 = outline
...
void main() {
    ...
    if (u_rainbow < 0.5) {
        ...
        // Outline: use control points directly (LINE_LOOP with 3 vertices)
        localPos = CTRL_PTS[gl_VertexID % NUM_CTRL];
    }
